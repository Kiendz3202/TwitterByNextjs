...
function Post({ id, post }) {
    ...
    const handlerDelete = async (e) => {
        e.stopPropagation()
        await deleteDoc(collection(db, "post", id))
        router.push(`/${id}`)
    }
        ...
        <div className='flex justify-start  px-4  min-h-[132px] h-full mt-5 border-b border-gray  '>
            ...
            <div className=''>
                ...
                <div className='flex justify-around items-center  w-full px-4 h-7  min-h-[32px] mb-2 '>
                    ...
                    {session.user.uid !== post?.id ? <AutorenewRoundedIcon className='cursor-pointer hover:text-red-500 text-base' /> : <span onClick={handlerDelete}><DeleteIcon className='cursor-pointer hover:text-red-500 text-base' /></span>}
