    ...
    return (
        <div className=' flex flex-col justify-center items-center xl:items-start xl:w-[260px] p-2 fixed h-full'>
            <ul className='h-full w-full'>
                ...
                <li className='hidden relative xl:block text-white mt-10 p-2 hover:opacity-80  hoverAnimation border-2 rounded-full  border-gray-400'>
                    <div onClick={openLogoutHanlder} className=' xl:flex h-7 items-center justify-center xl:justify-between  ' >
                        <Avatar src={session.image} />
                        <div className='mr-8'>
                            <h3 className=' text-sm'>{session.name}</h3>
                            <p className=' text-sm'>{"@" + session.tag}</p>
                        </div>
                        <MoreHorizIcon />
                    </div>
                </li>
                {openLogout && <div onClick={signoutHandler} className='absolute cursor-pointer hover:bg-opacity-80 flex justify-start items-center bottom-[70px] left-[20px] w-[224px] rounded-full  h-[30px] z-3 bg-gray-800'>
