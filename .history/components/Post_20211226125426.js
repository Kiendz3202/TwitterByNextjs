...
function Post({ id, post }) {
    ...
    const [binPermission, setBinPermission] = useState()
    ...
    // useEffect(() => onSnapshot(collection(db, "post", id, "comments"), (snapshot) => setComments(snapshot.docs)), [db, id])
    useEffect(
        () =>
            onSnapshot(
                query(
    ...
    useEffect(() => {
        if (session.user.uid === id) {
            setBinPermission(true)
        } else {
            setBinPermission(false)
        }
    }, [id])
        ...
        <div className='flex justify-start  px-4  min-h-[132px] h-full mt-5 border-b border-gray  '>
            ...
            <div className=''>
                ...
                <div className='flex justify-around items-center  w-full px-4 h-7  min-h-[32px] mb-2 '>
                    <span onClick={openModal}><ChatBubbleOutlineIcon className='cursor-pointer hover:text-red-500 text-base ' /> {comments.length > 0 && comments.length}</span>
                    {!binPermission ? <AutorenewRoundedIcon className='cursor-pointer hover:text-red-500 text-base' /> : <span onClick={handlerDelete}><DeleteIcon className='cursor-pointer hover:text-red-500 text-base' /></span>}
