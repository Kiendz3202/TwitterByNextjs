...
function Post({ id, post }) {
    ...
    useEffect(() => {
        onSnapshot(collection(db, "post", id, "likes"), (snapshot) => setLikes(snapshot.docs))
    }, [])

    useEffect(() => {
        setLiked(likes.findIndex((like) => (like.id === session?.user?.uid)))
    }, [])
    ...
    const likePost = async (e) => {
        ...
        if (liked) {
            ...
            await setDoc(doc(db, "post", id, "like", session.user.uid), {
