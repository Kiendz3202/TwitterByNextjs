...
function Widger({ trending, follow }) {
    return (
        <div className='hidden lg:flex flex-col items-center w-[400px]  pl-[68px]' >
            <div className='sticky top-0 z-20 h-[50px] flex  justify-start  items-center w-full bg-zinc-800 rounded-full '>
            ...
            <div className='sticky top-0 h-[380px] bg-zinc-800 w-full mt-3  rounded-2xl'>
                ...
                {/* <div className='sticky pt-[20px]  h-[280px] bg-zinc-800 w-full mt-3  rounded-b-2xl'>
                    {trending.map((trend) => <WidgerTrend key={trend} trendDetail={trend} />)}
                    <div className='absolute top-0  left-0 bg-black w-full h-[20px]'></div>
                </div> */}
            </div>
        </div>
    )
