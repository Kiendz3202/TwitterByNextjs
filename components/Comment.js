...
function Comment({ comment, idPost, id }) {
    ...
    useEffect(
        () =>
            onSnapshot(collection(db, "posts", idPost, "comments", id, "likesincomment"), (snapshot) =>
    ...
    const likePost = async (e) => {
        e.stopPropagation()
        if (liked) {
            await deleteDoc(doc(db, "posts", idPost, "comments", id, "likesincomment", session.user.uid));
        } else {
            await setDoc(doc(db, "posts", idPost, "comments", id, "likesincomment", session.user.uid), {
